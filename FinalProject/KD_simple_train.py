...
parser.add_argument('--model_save_dir', type=str, default='./results/KD_simple/')
...
# weights saving path
# model_save_dir = fr'./results/KD_simple/'
# dataset path
...
batch_size = 32
num_worker = 8
lr_steps = [(x+1) * 5 for x in range(100//5)]
gamma = 0.7
num_epoch = 70
print_freq = 500
...
for epoch_idx in range(start_epoch + 1, num_epoch + 1):
    ...
    for iter_idx, batch_data in enumerate(tqdm(train_dataloader)):
        ...
        # features MSE
        loss_mse = MSE(features, teacher_features)
