...
class KD_simple_v3(nn.Module):
    def __init__(self, student, in_features, out_features):
        super(KD_simple_v3, self).__init__()
        self.conv3_1 = nn.Sequential(
                        nn.Upsample(scale_factor=(4,4)),    
                        Conv1x1(in_features[2], out_features[0])
                     )   
        self.conv3_2 = nn.Sequential(
                        nn.Upsample(scale_factor=(2,2)),    
                        Conv1x1(in_features[2], out_features[1])
                     )   
        self.conv3_3 = Conv1x1(in_features[2], out_features[2])
        self.linear = linear(in_features[3], out_features[3])      
        self.student = student
        
    def forward(self, x):
        pred, features = self.student.forward(x, is_feat=True)
        feat1, feat2, feat3, feat = features
        feature1 = self.conv3_1(feat3)
        feature2 = self.conv3_2(feat3)
        feature3 = self.conv3_3(feat3)
...
def build_net(student=None):
    ...
    #2.5M
    # model = KD_simple_v3(student, in_features=[128, 256, 512, 704] , out_features=[40, 112, 320, 1280])
    #1.39M
    model = KD_simple_v3(student, in_features=[96, 192, 384, 480] , out_features=[40, 112, 320, 1280])
    return model